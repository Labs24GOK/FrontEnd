...
const AttendanceModal = props => {
  ...
  useEffect(() => {
    console.log(props.getStudentsByCourseID(props.staffID));
    ...
    setAttendance({ ...attendance, students: studentsNew });
  }, []);
  ...
  const changeHandler = (date, dateString) => {
    setAttendance({
      ...attendance,
      meeting: {
        course_id: props.studentList[0].course_id,
...
export default withRouter(
  connect(mapStateToProps, { getStudentsByCourseID, postStudentAttendance })(
    AttendanceModal
  )
);
