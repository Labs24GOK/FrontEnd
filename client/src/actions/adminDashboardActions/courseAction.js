...
export const getCourseTable = () => dispatch => {
    ...
    axios.get('http://localhost:4000/api?table=course_view')
        .then(res => {
...
export const getCourseById = id => dispatch => {
    ...
    axios.get(`http://localhost:4000/api?table=course_view&where=id=${id}`)
    .then(res => {
        dispatch({
...
export const editCouseById = (id, state) => dispatch => {
    axios.put(`http://localhost:4000/api?table=course_view&where=id=${id}`, state)
    .then(res => {
        dispatch({
...
export const addCourse = course => dispatch => {
    ...
    dispatch({ type: ADD_COURSE_START })
    axios.post('http://localhost:4000/api?table=course', newCourse)
        .then(res => {
            // if(res.status===201){
                // axios.get('http://localhost:4000/api?table=course_view').then(res => {
...
export const getDropDownCourses = () => dispatch => {
    ...
    const schoolGrade = axios.get(`http://localhost:4000/api/?table=school_grade`)
    const level = axios.get(`http://localhost:4000/api/?table=level`)
    const courseSchedule = axios.get(`http://localhost:4000/api/?table=course_schedule`)
