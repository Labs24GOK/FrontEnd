...
export const getStudentById = id => dispatch => {
  ...
  axios
    .get(`http://localhost:3001/api/?table=student&where=id=${id}`)
    .then(res => {
      dispatch({
...
export const editStudentById = (student_id, state) => dispatch => {
  ...
  axios
    .put(
      `http://localhost:3001/api/?table=student&where=id=${student_id}`,
      state
    )
    .then(res => {
      dispatch({
...
export const deleteStudentById = id => dispatch => {
  ...
  axios
    .put(`http://localhost:3001/api/?table=student&where=id=${id}`)
    .then(res => {
      dispatch({
