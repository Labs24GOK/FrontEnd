...
export const getStudentById = id => dispatch => {
    ...
    axios.get(`http://localhost:4000/api/?table=student&where=id=${id}`)
    .then(res => {
        dispatch({
...
export const editStudentById = (student_id, state ) => dispatch => {
    ...
    axios.put(`http://localhost:4000/api/?table=student&where=id=${student_id}`, state)
    .then(res => {
        dispatch({
    ...
    .catch(err => {
       dispatch({
        ...
        payload: 'Error saving changed student information', err
       }) 
    })
}
...
export const deleteStudentById = id => dispatch => {
    ...
    axios.put(`http://localhost:4000/api/?table=student&where=id=${id}`)
    .then(res => {
        dispatch({
...
export const editStudentDropDown = () => dispatch => {
    ...
    const contactTable = axios.get(`http://localhost:4000/api/?table=preferred_contact_type`)
