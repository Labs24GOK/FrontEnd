...
export const createNewStudent = student => dispatch => {
	...
	axios
		.post(`${API_URL}/api/?table=student`, newStudent)
		.then(res => {
...
export const getDropDown = () => dispatch => {
	const locationTable = axios.get(`${API_URL}/api/?table=location`);
	const contactTable = axios.get(
		`${API_URL}/api/?table=preferred_contact_type`
	);
	const gradeTable = axios.get(`${API_URL}/api/?table=school_grade`);
	const blockTable = axios.get(`${API_URL}/api/?table=block`);
