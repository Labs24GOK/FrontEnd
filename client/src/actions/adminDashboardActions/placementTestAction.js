...
export const getPlacementTests = () => dispatch => {
  ...
  axios
    .get('http://localhost:3001/api?table=placementexam')
    .then(res => {
      dispatch({
...
export const getPlacementTestById = id => dispatch => {
  ...
  axios
    .get(
      `http://localhost:3001/api/?table=placementexam&where=student_id=${id}`
    )
    .then(res => {
      dispatch({
...
export const editPlacementTestById = (id, state) => dispatch => {
  axios
    .put(
      `http://localhost:3001/api/?table=student&where=student_id=${id}`,
      state
    )
    .then(res => {
